			...
			<div id="content">
				...
				<div class="container-fluid">
					...
					<div class="d-sm-flex align-items-center justify-content-between mb-5 mt-5">
						<h1 class="h3 ml-3 mb-0 text-gray-800">활동 정지 회원 리스트</h1>
					</div>
						...
						<div class="table-responsive">
							...
							<thead class="table-primary">
								<tr>
  									...
  									<th class="w-15">이름</th>
  									<th class="w-30">정지일</th>
  									<th class="w-25">정지기간</th>
  									<th class="w-15"></th>
  								</tr>
  							</thead>
  							<tbody>
  								<c:forEach var="list" items="${userStopList }">
  									<tr>
  										<td>${list.id }</td>
  										<td>${list.name }</td>
  										<td>${list.insDate }</td>
  										<td>${list.stopNum }일 (${list.stopNumDate })</td>
  										<c:choose>
  											<c:when test="${list.stopEnd == -1 }">
		  										<td>
			  										<input type="button" class="btn btn-outline-primary btn-sm" value="정지해제" onclick="location.href='<%=cp %>/userstopdone.action?code=${list.code }'"/>
			  									</td>
  											</c:when>
  											<c:otherwise>
  												<td>
  													<input type="button" class="btn btn-outline-primary btn-sm" value="정지종료" disabled="disabled"/>
  												</td>
  											</c:otherwise>
  										</c:choose>
  									</tr>
  								...
  								<!-- <tr>
  									...
  									<td>김철수</td>
  									<td>2020-06-11</td>
  									<td>14일(2020-06-24)</td>
  									<td>
  										<input type="button" class="btn btn-outline-primary btn-sm" value="정지해제"/>
  									</td>
  								</tr> -->
  							</tbody>
  							</table>
